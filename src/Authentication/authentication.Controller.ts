...
export class Authentication {
    ...
    async login(req: Request, res: Response) {
        ...
        const limiter = await this.login_fails_limit.getInstance(req.body.email);
        if (limiter && limiter.reachedLimit) {
            return res.send('Reached Limit Points');
        }

        const isValidSheama = loginSheama.validate(req.body);
        if (isValidSheama == false) {
            return res.send('Payload Recieved is not valid');
        }

        const client = await this.clientModule.getUserByEmail(req.body.email);
        if (client == undefined) {
        ...
        if (limiter == undefined) {
            await this.login_fails_limit.insert(req.body.email);
            console.log('--> Create New Limiet For Client');
        }
        else {
            await this.login_fails_limit.increment(req.body.email);
